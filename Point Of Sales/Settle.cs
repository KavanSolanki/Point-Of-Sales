    ...
    public partial class Settle : Form
    {
        ...
        private void btnEnter_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                else
                {
                    for (int i = 0; i < cashier.dgvCash.Rows.Count; i++)
                    {
                        ...
                        cn.Open();
                        cm = new SqlCommand("UPDATE tblCart SET status = 'Sold' WHERE id= '" + cashier.dgvCash.Rows[i].Cells[1].Value.ToString() + "'", cn);
                        cm.ExecuteNonQuery();
                        cn.Close();
                    }
            ...
            catch (Exception ex)
            {
                ...
                dbcon.Error("Settle/btnEnter_Click", ex.Message, linenumber);
            }
        }
